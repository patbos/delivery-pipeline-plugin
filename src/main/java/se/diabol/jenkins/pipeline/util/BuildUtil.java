    ...
    public static AbstractBuild getUpstreamBuild(AbstractBuild build) {
        ...
        for (CauseAction action : actions) {
            ...
            for (Cause.UpstreamCause upstreamCause : causes) {
                AbstractProject upstreamProject = JenkinsUtil.getInstance()
                        .getItemByFullName(upstreamCause.getUpstreamProject(), AbstractProject.class);
                //Due to https://issues.jenkins-ci.org/browse/JENKINS-14030 when a project has been
                // renamed triggers are not updated correctly
